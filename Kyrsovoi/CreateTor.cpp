...
IMPLEMENT_DYNAMIC(CCreateTor, CDialogEx)

CCreateTor::CCreateTor(CWnd* pParent /*=NULL*/)
	: CDialogEx(CCreateTor::IDD, pParent)
{

}

CCreateTor::~CCreateTor()
{
}

void CCreateTor::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RAZB_VERT, RAZB_VERT);
...
BEGIN_MESSAGE_MAP(CCreateTor, CDialogEx)
	ON_BN_CLICKED(IDOK, &CCreateTor::OnBnClickedOk)
	ON_BN_CLICKED(IDCANCEL, &CCreateTor::OnBnClickedCancel)
END_MESSAGE_MAP()


// обработчики сообщений CCreateTor

void CCreateTor::Set_first(dra* first)
{
	f=first;
}

void CCreateTor::Get_Numb(int N)
{
	Number=N;
}
BOOL CCreateTor::OnInitDialog()
{
...
void CCreateTor::OnBnClickedCancel()
{
	Ex=0;
	CDialogEx::OnCancel();
}
